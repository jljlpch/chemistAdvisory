    	...
    	if(this.state.userName==''||this.state.code==''||this.state.passWord==''||this.state.repassWord==''){
			...
			if(this.state.passWord.length<6){
	    		...
	    		if(this.state.passWord==this.state.repassWord){
			    	...
			    	var str=API.resetpasswordphone.split('?')[1]+'&code='+this.state.code+'&cellphone='+this.state.userName+'&passWord='+passWord;
	    			var sign=encryption(str);
			    	ajax().post(API.resetpasswordphone,{ code:this.state.code,cellphone:this.state.userName,passWord:passWord,sign:sign },'json')
		    		.success(function(data){
		    			console.log(data);
		    			if(data.code==0){
		    				window.location.href='#/login?findBack';	
		    			}else{
    ...
    codeClick:function(){
    	if(this.state.userName.match(/^(1[3,5,8,7]{1}[\d]{9})/)&&(this.state.codeContent=='获取验证码')){//如果是正确的电话号码
	    	...
	    	var interval=setInterval(function(){//60s计时动画
	    		if(!location.hash.match('findBack')){//如果不在该页面就清除定时器
	    	...
	    	var str=API.getcellphonecode.split('?')[1]+'&cellphone='+this.state.userName+'&type='+'1';
	    	var sign=encryption(str);
	    	ajax().post(API.getcellphonecode,{ cellphone:this.state.userName,type:'1',sign:sign },'json');
    	}else{
